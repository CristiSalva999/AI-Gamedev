...
- No local LLM or Blender exists in the cloud VM, and that is fine: the app is designed to run fully offline.
  ...
  - Configure the real model via env (`LLM_BASE_URL`, `LLM_API_KEY`, `LLM_MODEL`); see `.env.example`. Copy to `server/.env` (loaded automatically via dotenv). LM Studio ignores the API key value. Base URLs without `/v1` (pi.dev style `http://127.0.0.1:1234`) are normalized to `.../v1`.
  - **Local PC (not Cloud):** LM Studio + Blender on the user's machine are unreachable from the cloud VM. Run `npm run dev` on the PC with LM Studio's local server started and `BLENDER_BIN` pointing at `blender.exe` to get `source: "llm"` and `blender.mode: "blender"`.
