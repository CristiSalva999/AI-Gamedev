...
- Playable preview: cinematic lighting (ACES), procedural material maps, terrain, walk **or drive** controls from `player.avatar`. Press **E** to collect (exploration) or drive through checkpoints (racing). Click the canvas first for keyboard focus. Packaged `play.html` remains the offline runner.
