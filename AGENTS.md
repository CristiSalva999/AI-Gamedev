...
- **Multi-genre cinematic pipeline**: `shared/src/gameDesign.ts` + `server/src/pipeline/genrePacks.ts` define structured `GameDesignDoc` / `WorldRecipe` per genre (exploration, racing, shooter, dungeon…). The design/world stages ask the local LLM for rich JSON; mocks supply the same shapes offline. Default fidelity is **cinematic** (not low-poly).
- **Compound prefabs + detail enrichment** (`shared/src/prefabs.ts`, `shared/src/detail.ts`): multi-mesh set pieces with material hints; `enrichDefinition` adds micro-detail for cinematic builds. Racing packs include cars, barriers, checkpoints, grandstands.
- **Terrain** (`shared/src/terrain.ts`): deterministic heightfields (`rolling`, `track_bowl`, …) shared by layout Y and the Three.js mesh.
- **Control profiles** (`shared/src/controls.ts`): each build gets a `blueprint.controls` map from the design `controlScheme` — `drive` (W/S accel, A/D steer, Space handbrake, Shift boost), `fps` (WASD, Space fire, R reload, …), `walk` (WASD, E collect, Shift sprint, Space jump), etc. The viewport and HUD read that profile; do not hard-code one binding set for every genre.
- **Prefill scaffold** (`server/src/pipeline/scaffold.ts` + `scriptAuthor.ts`): every build gets a complete `blueprint.runtime` (objectives, HP/ammo/lives, win/lose, race/combat rules) modeled on the user prompt (e.g. “3 giri”, “hard”, “collect 4 relics”). `gameplay.ts` is authored as a full rules module from that scaffold (LLM may append polish); the preview executes the same session logic via `shared/src/sessionLogic.ts`.
- Playable preview: cinematic lighting (ACES), procedural material maps, terrain, scheme-specific controls. Click the canvas first for keyboard focus. Packaged `play.html` remains the offline runner.
