...
- **Compound prefabs** (`shared/src/prefabs.ts`): asset briefs map to multi-mesh set pieces (trees, stone arches, wells, statues…) via `AssetSpec.parts`. The Three.js preview builds Groups from those parts — not single primitives. Forest/ruins prompts also get a themed layout (ruin cluster + path + tree ring) and ambient fillers.
- Playable preview: the viewport renders the blueprint (themed lighting, compound prefabs, soft collision) with a WASD/arrow-key player; press **E** near interactive props to collect loot. Click the canvas first to give it keyboard focus. After packaging, use **⤓ install zip** in the chat panel (or curl the artifact URL) and open `play.html` locally — no install step.
